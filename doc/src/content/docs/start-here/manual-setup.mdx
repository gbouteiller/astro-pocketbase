...
Most package managers will install associated peer dependencies as well. If you see a `"Cannot find package 'pocketbase'"` (or similar) 
warning when you start up Astro, you'll need to install `pocketbase` and `@11ty/eleventy-fetch` as well:

<Tabs>
  <TabItem label="npm">
    ```shell
    npm install pocketbase @11ty/eleventy-fetch
    ```
  </TabItem>
  <TabItem label="pnpm">
    ```shell
    pnpm add pocketbase @11ty/eleventy-fetch
    ```
  </TabItem>
  <TabItem label="yarn">
    ```shell
    yarn add pocketbase @11ty/eleventy-fetch
    ```
  </TabItem>
</Tabs>
